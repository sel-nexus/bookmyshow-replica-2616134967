...
function PaymentContent() {
  ...
  async function submitPayment(paymentMethod: PaymentMethod): Promise<Booking> {
    ...
    const booking = await createBooking(token, { movieId, theatreId, seats, paymentMethod, totalPrice });
    sessionStorage.setItem('bookmyshow:lastBooking', JSON.stringify(booking));
    router.push(`/confirmation?movieId=${movieId}&theatreId=${theatreId}`);
    return booking;
  }

  if (isAuthLoading || !user) return <main className="welcome"><p role="status">Loading your session...</p></main>;
  if (!validContext) return <main className="welcome"><p className="form-error" role="alert">Your seat selection is incomplete. Please start again.</p><Link className="back-link" href="/dashboard">← Back to movies</Link></main>;

  return (
    <main className="page-shell">
      <header className="site-header"><Link className="brand" href="/"><span className="brand-mark">B</span><span>bookmyshow replica</span></Link><span className="header-note">Step 2 of 2</span></header>
      <section className="welcome" aria-labelledby="payment-heading">
        <span className="eyebrow">Almost there</span>
        <h1 id="payment-heading">Make it official.</h1>
        <p>{seats.join(', ')} · Rs. {totalPrice}</p>
        {error && <p className="form-error" role="alert">{error}</p>}
        <div className="focal-panel" style={{ marginTop: 34 }}><PaymentForm onSubmit={submitPayment} /></div>
      </section>
    </main>
  );
}
...
export default function PaymentPage() {
  return <Suspense fallback={<main className="welcome"><p role="status">Loading payment...</p></main>}><PaymentContent /></Suspense>;
}
